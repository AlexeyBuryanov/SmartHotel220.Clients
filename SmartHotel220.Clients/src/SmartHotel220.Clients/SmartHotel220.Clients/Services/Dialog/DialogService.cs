...
namespace SmartHotel220.Clients.Core.Services.Dialog
{
    ...
    public class DialogService : IDialogService
    {
        ...
        {
            return SelectActionAsync(message, title, "Отмена", options);
        }
        ...
        public async Task<string> SelectActionAsync(string message, string title, string cancelLabel, IEnumerable<string> options)
        {
            try
            {
                ...
                if (!options.Any())
                {
                    throw new ArgumentException("Нет доступных опций", nameof(options));
                }

                var result = await UserDialogs.Instance.ActionSheetAsync(message, cancelLabel, null, 
